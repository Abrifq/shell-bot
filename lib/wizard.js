...
function stepEnableMultiOwner(rl, config) {
    var prompt = "Would you like to enable multiple \"owner\" support?\n" +
        "This will allow multiple people you specified execute commands in the shell.\n" +
        "Answer [y/n]: ";
    return question(rl, prompt).then(
        function (answer) {
            console.log();
            answer = answer.trim().toLowerCase();
            if (answer === "y" || answer === "yes")
                config.useMultipleOwners = true;
            else
                config.useMultipleOwners = false;
        }
    );
}
...
function stepOwners(rl, config) {
    var prompt = "Enter an id to add to the list or enter blank to exit: ";
    const handleAnswer = function (answer) {
        if (answer.trim() === "")
            return;
        else {
            config.owners.push(answer.trim());
            return stepOwners(rl, config);
        }
    };
    return question(rl, prompt).then(handleAnswer);
}
...
function configWizard(options) {
    ...
    .then(function (bot_) {
        ...
        .then(stepEnableMultiOwner)
    .then(function () {
        if (config.useMultipleOwners) {
            config.owners = [];
            return stepOwners(rl, config);
        }
        else {
